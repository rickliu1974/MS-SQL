USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_Stock_taking_PL13]    Script Date: 08/18/2017 17:43:41 ******/
DROP PROCEDURE [dbo].[uSP_Imp_Stock_taking_PL13]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_Stock_taking_PL13]
as
begin
  /***********************************************************************************************************
     2013/12/06 -- Rickliu
  ...
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_Stock_taking_PL13'
  Declare @Cnt Int =0
...
print '1'
  ...
  Set @TB_head = 'Stock_taking_PL13'
  Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#Stock_taking_PL13 -- Excel 原檔資料
  Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: Stock_taking_PL13_tmp -- 臨時轉入介面
  Set @TB_SP_Name = @TB_Head -- Ex: Stock_taking_PL13
  set @SP_Class = '7' -- 盤點類
...
print '10'
  ...
  Declare Cur_Stock_taking_PL13 cursor for
    select distinct f3 as SP_whno
      from [DW].[dbo].[Ori_xls#Stock_taking_PL13]
	 where f3 <> '盤點員' and f3 <> '倉別'

  open Cur_Stock_taking_PL13
  fetch next from Cur_Stock_taking_PL13 into @SP_whno
...
print '25'
        ...
        set @strSQL ='delete TYEIPDBS2.lytdbpl13.dbo.sslpdttmp '+@CR+
...
print '26'
        ...
        set @strSQL ='delete TYEIPDBS2.lytdbpl13.dbo.ssliptmp '+@CR+
...
print '27'
        ...
        set @strSQL ='insert into TYEIPDBS2.lytdbpl13.dbo.sslpdttmp '+@CR+
                     ...
                     '       inner join SYNC_PL13.dbo.sstock d '+@CR+
                     ...
                     '            from SYNC_PL13.dbo.swaredt m'+@CR+
                     '                 inner join '+@CR+
                     '                   (select wd_no, wd_skno, max(wd_yr) as wd_yr '+@CR+
                     '                      from SYNC_PL13.dbo.swaredt '+@CR+
...
print '29'
        ...
        set @strSQL ='insert into TYEIPDBS2.lytdbpl13.dbo.ssliptmp '+@CR+
                     ...
                     '  from TYEIPDBS2.lytdbpl13.dbo.sslpdttmp m '+@CR+
                     '       left join SYNC_PL13.dbo.sstock d '+@CR+
...
proc_exit:
print '32'
  close Cur_Stock_taking_PL13
  deallocate Cur_Stock_taking_PL13
  set @Cnt = 0
  set @SP_Date  = ''
