USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_CrazyMike]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_CrazyMike]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_CrazyMike]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.CrazyMike_Order 為 瘋狂賣客EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_CrazyMike'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_CrazyMike'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_CrazyMike -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_CrazyMike_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_CrazyMike -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_CrazyMike_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F23:規格原廠國際條碼'
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F22,1,1),'''''''','''') as F4, '+@CR+  --商品編號
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F4,1,1),'''''''','''') as F5, '+@CR+  --商品名稱
                  '       dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F6,1,1) as F6, '+@CR+  --訂貨量  
                  '       F8 as F7, '+@CR+  --贈品否  
                  '       dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F6,1,1) as F8, '+@CR+  --揀貨量  
                  '       dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F7,1,1) as F9, '+@CR+  --進價    
                  ...
                  '       replace(F11,'''''''','''') as buyer, '+@CR+
                  '       replace(F11,'''''''','''') as cust, '+@CR+
                  '       replace(F13,'''''''','''') as cust_phone, '+@CR+ 
                  '       replace(F13,'''''''','''') as cust_mobile, '+@CR+ 
                  '       replace(F12,'''''''','''') as cust_addr1, '+@CR+ 
                  '       replace(F12,'''''''','''') as cust_addr2, '+@CR+ 
                  '       replace(F14,'''''''','''') + '' '' + replace(F15,'''''''','''') as MEMO, '+@CR+ 
