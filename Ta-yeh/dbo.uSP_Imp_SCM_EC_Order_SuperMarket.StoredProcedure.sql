USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_SuperMarket]    Script Date: 08/18/2017 17:43:40 ******/
...
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_SuperMarket]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.YahooMall_Order 為 Yahoo超級商城EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_SuperMarket'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_SuperMarket'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_SuperMarket -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_SuperMarket_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_SuperMarket -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_SuperMarket_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F29:商品稅別      '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F10,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F11 as F5, '+@CR+  --商品名稱
                  ...
                  '       F2 as buyer, '+@CR+
                  '       F4 as cust, '+@CR+
                  '       F18 as cust_phone, '+@CR+ 
                  '       F20 as cust_mobile, '+@CR+ 
                  '       F5+F6 as cust_addr1, '+@CR+ 
                  '       F5+F6 as cust_addr2, '+@CR+ 
                  '       F26 as MEMO, '+@CR+ 
    ...
    Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
                  ...
                  '       F9=case when PATINDEX(''%[0-9]%'',F9)>0 then (Convert(Numeric(20, 6), F9) + F8) /1.05 else 0 end, '+@CR+ --進價
