USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_UDN]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_UDN]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_UDN]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.UDN_Order 為 UDN EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_UDN'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_UDN'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_UDN -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_UDN_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_UDN_Order_Orde  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_UDN_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F28:貨運公司          '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  -- 2017/07/10 Rickliu 變更商品欄位 原 P 欄(F16 商品型號) 改為 O 欄(F15 商品料號)
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F15, 1, 1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F18 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F20, 1, 1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F23 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F20, 1, 1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F23, 1, 1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F3,1,1),'' '','''') as source_order, '+@CR+ --訂單編號
                  '       F6 as buyer, '+@CR+
                  '       F7 as cust, '+@CR+
                  '       F8 as cust_phone, '+@CR+ 
                  '       F9 as cust_mobile, '+@CR+ 
                  '       F10+F11 as cust_addr1, '+@CR+ 
                  '       F10+F11 as cust_addr2, '+@CR+ 
                  '       F12 +''&'' + F13 as MEMO, '+@CR+ 
