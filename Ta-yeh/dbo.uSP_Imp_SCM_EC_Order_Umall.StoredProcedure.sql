USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_Umall]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_Umall]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_Umall]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.Umall_Order 為 森森購物EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_Umall'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_Umall'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_Umall -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_Umall_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_Umall -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_Umall_tmp --< 此資料存放於 tempdb，程序離開時會清除。
