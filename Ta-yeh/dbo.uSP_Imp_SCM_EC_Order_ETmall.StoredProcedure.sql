USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_ETmall]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_ETmall]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_ETmall]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.ETmall_Order 為 東森得易購EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_ETmall'
  Declare @Cnt Int =0
  ...
  Set @CompanyName = '東森' --> 請勿亂變動
    ...
    Set @TB_head = 'EC_Order_ETmall'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_ETmall -- Text 原檔資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_ETmall_tmp -- 臨時轉入介面
    Set @TB_OR_Name = @TB_Head -- Ex: EC_ETmall_Order  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_ETmall_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print 'F31：預計入庫日期'
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F29 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F13,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F15,1,1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F2,1,1),'' '','''') as source_order, '+@CR+ --指定到貨日    
                  '       F16 as buyer, '+@CR+
                  '       F16 as cust, '+@CR+
                  '       F17 as cust_phone, '+@CR+ 
                  '       F17 as cust_mobile, '+@CR+ 
                  '       F19 as cust_addr1, '+@CR+ 
                  '       F19 as cust_addr2, '+@CR+ 
                  '       F25 as MEMO, '+@CR+ 
