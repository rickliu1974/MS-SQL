USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_SharedFormat]    Script Date: 08/18/2017 17:18:56 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_SharedFormat]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_SharedFormat]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.EC_SharedFormat 為部分 EC 共同轉檔格式，目前有 露天、宸實(小p)、大買家 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_SharedFormat'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_SharedFormat'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_SharedFormat -- 外部匯入原檔資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_SharedFormat_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_SharedFormat_Order -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_SharedFormat_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F12:備註          '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F2, '+@CR+  --店名
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F8,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F9 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F10,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F11 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F10,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F11,1,1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F2,1,1),'' '','''') as source_order, '+@CR+ --訂單編號   
                  '       F3 as buyer, '+@CR+
                  '       F4 as cust, '+@CR+
                  '       F5 as cust_phone, '+@CR+ 
                  '       F6 as cust_mobile, '+@CR+ 
                  '       F7 as cust_addr1, '+@CR+ 
                  '       F7 as cust_addr2, '+@CR+ 
                  '       F12 as MEMO, '+@CR+ 
    ...
    Set @strSQL = ';With QTE_Q1 as( '+@CR+
                  ...
                  '     and or_rem like '''+@CompanyLikeName+''' '+@CR+
                  '   union '+@CR+
                  '  select distinct rtrim(or_wkno) as or_wkno '+@CR+
    ...
    begin
       ...
       Exec uSP_Sys_Write_Log @Proc, @Msg, @strSQL, 1
...
--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    set @strSQL = ';With CTE_Q1 as ( '+@CR+
                  ...
                  '          on m.ct_no collate Chinese_Taiwan_Stroke_CI_AS like ''%''+rtrim(d.f2)+''%'' collate Chinese_Taiwan_Stroke_CI_AS '+@CR+
