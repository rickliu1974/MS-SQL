USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_Comp_EC_Consign_Stock_Yahoo]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_Comp_EC_Consign_Stock_Yahoo]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_Comp_EC_Consign_Stock_Yahoo]
as
begin
  /***********************************************************************************************************
     ...
     2013/11/28 增加失敗回傳值
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_Comp_EC_Consign_Stock_Yahoo'
  Declare @Cnt Int =0
  ...
  Set @CompanyLikeCode = 'I90020013'
  Set @CompanyName = 'YAHOO'
...
print '4'
  ...
  Set @TB_xls_Name = Isnull(@xls_head+@TB_Head, '') -- Ex: Ori_Xls#Comp_EC_Consign_Stock_Yahoo -- Excel 原檔資料
...
print '19'
    ...
    set @strSQL = 
              ...
              -- 客戶編號
              '  from (Select F2 as xls_skno, F4 as xls_skname, F3 as xls_cono, '+@CR+ -- F2:商品條碼(Excel), F4: 商品名稱, F3: 輔助編號
              '               Sum(isnull(Convert(Numeric(20, 6), Replace(F24, '','', '''')), 0)) as xls_qty '+@CR+ -- 寄賣庫存 (Excel)
              '          from '+@TB_tmp_Name+@CR+
              '         where RTrim(F2) <> '''' '+@CR+
