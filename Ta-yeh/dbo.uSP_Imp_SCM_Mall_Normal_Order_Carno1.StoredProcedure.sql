USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_Mall_Normal_Order_Carno1]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_Mall_Normal_Order_Carno1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_Mall_Normal_Order_Carno1]
as
begin
  /***********************************************************************************************************
     2014/01/08 -- Rickliu
     車之輪 後台拉單一律轉訂單，訂單日期一律抓取製表日期，而 指定到貨日 則寫入 訂單的交貨日期
     切記：
     1.車之輪不願意配合 3C 與 百貨 進行拆單作業，所以統一轉入百貨客戶，由客服自行拆單。
     2.此程序僅處理客戶揀貨所產生的揀貨單，透過此單轉成我司之訂單，除此以外則不在此處理範圍內。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_Mall_Normal_Order_Carno1'
  Declare @Cnt Int =0
  Declare @RowCnt Int =0
  Declare @Msg Varchar(4000) =''
  Declare @Errcode int = -1

  Declare @Kind Varchar(1), @KindName Varchar(10), @OR_Class Varchar(1), @OR_CName Varchar(20)
...
print '2'
  ...
  Set @TB_head = 'Mall_Normal_Order_Carno1'
  Set @TB_txt_Name = @txt_head+@TB_Head -- Ex: Ori_txt#Mall_Normal_Order_Carno1 -- Text 原檔資料
  Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: Mall_Normal_Order_Carno1_tmp -- 臨時轉入介面
  Set @TB_OR_Name = @TB_Head -- Ex: Mall_Normal_Order_Carno1  
...
print '12'
     ...
     -- '   set F2=Replace(Replace(REPLACE(F1, '' '', ''''), ''店)廠商揀貨用(不送門市)'', ''''), ''('', '''') '+@CR+
                   '   set F2=Replace(Replace(Replace(REPLACE(F1, '' '', ''''), ''店)廠商揀貨用(不送門市)'', ''''), ''門市)廠商揀貨用(不送門市)'', ''''), ''('', '''') '+@CR+
                   ' where F1 like ''%'+SPACE(20)+'(%'''
     Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
...
print '18'
     ...
     set @strSQL = 'select rowid, ct_no, ct_sname, ct_ssname, '+@CR+
                   ...
                   -- 單價
                   '       case when (Convert(Numeric(20, 6), F6)) = 0 then 0 else ((Convert(Numeric(20, 6), F10)) / (Convert(Numeric(20, 6), F6))) end as Ori_sale_amt, '+@CR+ -- 2015/08/19 Rickliu 保留原本欄位，避免後面修改
                   '       case when (Convert(Numeric(20, 6), F6)) = 0 then 0 else ((Convert(Numeric(20, 6), F10)) / (Convert(Numeric(20, 6), F6))) end as sale_amt, '+@CR+
                   -- 小計
                   ...
                   '                   and substring(ct_no, 9, 1) =''1'' '+@CR+ -- 1:百貨, 2:3C, 3:寄倉, 4:代工OEM
                   '                ) m, ['+@TB_tmp_Name+'] d '+@CR+
...
print '22'
        ...
        --set @strSQL ='delete TYEIPDBS2.lytdbta13.dbo.sorddttmp '+@CR+
...
print '23'
        ...
        --2014/04/22 副總指示直接轉確認訂單，無須再行審核。
        --set @strSQL ='delete TYEIPDBS2.lytdbta13.dbo.sordertmp '+@CR+
...
print '24'
        ...
        --2014/04/22 副總指示直接轉確認訂單，無須再行審核。
        --set @strSQL ='insert into TYEIPDBS2.lytdbta13.dbo.sordertmp '+@CR+
        ...
        set @strSQL ='insert into TYEIPDBS2.lytdbta13.dbo.sorder '+@CR+
                     ...
                     '       1 as or_rate, '+@CR+ -- 匯率
                     '       Substring(M.F12+'':''+Convert(Varchar(10), rowid), 1, 10) as or_wkno, '+@CR+ -- 客戶訂單單號
                     ...
                     '       left join  '+@CR+
                     '        (select ct_no, F12, '+@CR+
                     '                convert(varchar(10), '+@New_Orno+'+row_number() over(order by m.ct_no, F12)) as od_no '+@CR+ -- 貨單編號
                     '           from (select distinct m.ct_no, F12 '+@CR+
                     '                   from ['+@TB_OR_Name+'] m  '+@CR+
                     '                        left join SYNC_TA13.dbo.pcust d1 '+@CR+
                     '                          on m.ct_no = d1.ct_no '+@CR+
                     '                         and ct_class =''1'' '+@CR+
                     ...
                     '         ) d3 on d1.ct_no = d3.ct_no '+@CR+
                      '         and m.F12 = d3.F12 '+@CR+
                     --20161219 add by NanLiao
                     '       left join (select ct_no , F12, count(1) as cnt from ['+@TB_OR_Name+'] group by ct_no, F12) d4 '+@CR+
                     ...
                     '       Substring(M.F12+'':''+Convert(Varchar(10), rowid), 1, 10), '+@CR+ -- 客戶訂單單號
                     '       d4.cnt, '+@CR+ -- 明細筆數
                     '       d1.ct_payfg '
        Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
        ...
        --2014/04/22 副總指示直接轉確認訂單，無須再行審核。
        --set @strSQL ='insert into TYEIPDBS2.lytdbta13.dbo.sorddttmp '+@CR+
