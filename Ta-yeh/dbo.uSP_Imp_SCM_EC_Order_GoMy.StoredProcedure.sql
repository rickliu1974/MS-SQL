USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_GoMy]    Script Date: 08/18/2017 17:18:56 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_GoMy]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_GoMy]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.GoMy_Order 為 GoMy EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_GoMy'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_GoMy'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_GoMy -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_GoMy_tmp -- 待轉入凌越資料介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_GoMy --< 此資料存放於 tempdb，程序離開時會清除。
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_GoMy_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F18:合計金額      '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F5 as cust, '+@CR+
                  '       F6 as cust_phone, '+@CR+ 
                  '       F7 as cust_mobile, '+@CR+ 
                  '       F8+F9 as cust_addr1, '+@CR+ 
                  '       F8+F9 as cust_addr2, '+@CR+ 
                  '       F11 +''  '' + F12 as MEMO, '+@CR+ 
