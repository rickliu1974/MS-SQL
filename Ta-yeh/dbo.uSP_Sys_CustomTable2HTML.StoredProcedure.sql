USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Sys_CustomTable2HTML]    Script Date: 08/18/2017 17:18:56 ******/
DROP PROCEDURE [dbo].[uSP_Sys_CustomTable2HTML]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/****** Object:  StoredProcedure [dbo].[uSP_Sys_CustomTable2HTML]    Script Date: 12/21/2011 09:04:30 ******/
CREATE PROCEDURE [dbo].[uSP_Sys_CustomTable2HTML] (@TABLENAME  NVARCHAR(500), @OUTPUT NVARCHAR(MAX) OUTPUT, @TBL_STYLE NVARCHAR(1024) = '', @ALIGNMENT INT =0 )
AS
...
-- @ParmDefinition stores the parameter definition for the dynamic SQL
  ...
  IF @TBL_STYLE = ''
     Set @TBL_STYLE = 'style="font:14pt" cellpadding="2" cellspacing="1" border="1"'
...
'     SELECT @maxrows=count(1) FROM #CustomTable2HTML '+
...
'          SET @exec_str=''SELECT @ROWOUT=(select ['' + @ColumnName + ''] from #CustomTable2HTML where CustColHTML_ID=@rowNum_IN)''
           ...
           Exec SP_executesql @exec_str, @ParmDefinition, @ROWOUT=@ROW OUTPUT, @rowNum_IN=@rownum
...
'       SET @ROW=''''
        ...
        IF @DEBUG=1 PRINT ''@exec_str: '' + @exec_str
        Exec SP_executesql @exec_str, @ParmDefinition, @COLOUT=@ROW OUTPUT
...
--a parameter to this SP)
  Exec SP_executesql @exec_str, @ParamDefinition, @OUTPUT=@OUTPUT OUTPUT
