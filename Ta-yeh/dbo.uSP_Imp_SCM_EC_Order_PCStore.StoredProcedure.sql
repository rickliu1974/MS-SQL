USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_PCStore]    Script Date: 08/18/2017 17:18:56 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_PCStore]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_PCStore]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.PCStore_Order 為 Pchome商店街EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_PCStore_Order'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_PCStore'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_PCStore -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_PCStore_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_PCStore  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_Order_PCStore_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F14:店家料號          '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F14,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       replace(F9,'''''''','''') as F5,  '+@CR+  --商品名稱
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F1,1,1),'' '','''') as source_order, '+@CR+  
                  '       replace(F3,'''''''','''') as buyer, '+@CR+
                  '       replace(F4,'''''''','''') as cust, '+@CR+
                  '       replace(F8,'''''''','''') as cust_phone, '+@CR+ 
                  '       replace(F8,'''''''','''') as cust_mobile, '+@CR+ 
                  '       replace(F6,'''''''','''')+ '' '' + replace(F7,'''''''','''')  as cust_addr1, '+@CR+ 
                  '       replace(F6,'''''''','''')+ '' '' + replace(F7,'''''''','''')  as cust_addr2, '+@CR+ 
                  '       replace(F13,'''''''','''') as MEMO, '+@CR+ 
