USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_Mall_Normal_Order_Car1]    Script Date: 07/24/2017 14:43:59 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_Mall_Normal_Order_Car1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_Mall_Normal_Order_Car1]
as
begin
  /***********************************************************************************************************
     2013/12/04 -- Rickliu
  ...
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_Mall_Normal_Order_Car1'
  Declare @Cnt Int =0
  ...
  Declare @Or_DateKind varchar(1)  --add by Nan Liao 2015/12/07 配合轉檔時，判斷列印日期欄位
  ...
  Declare @RowData Table (aData Varchar(255),bData Varchar(1))--add by Nan Liao 2015/12/07 配合轉檔時，判斷列印日期欄位
  ...
  set @Last_Date = '25'
  set @Or_Date1 = Substring(Convert(Varchar(10), getdate(), 111), 1, 8)+@Last_Date

  
  Set @CompanyName = '車麗屋' --> 請勿亂變動
  ...
  --*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*
print '1'       
  Declare Cur_Mall_Normal_Order_Car1_DataKind cursor for
    select *
      from (select '3' as Kind, 'Retail' as KindName
             union
            select '2' as Kind, '3c' as KindName
           )m,
           (select 'Order' as OR_Name, '揀貨轉訂單' as OR_CName, '3' as OR_Class
           )d
    order by kind
     
  open Cur_Mall_Normal_Order_Car1_DataKind
  fetch next from Cur_Mall_Normal_Order_Car1_DataKind into @Kind, @KindName, @OR_Name, @OR_CName, @OR_Class
...
print '3'
    ...
    Set @TB_head = 'Mall_Normal_Order_Car1_'
    Set @TB_xls_Name = @xls_head+@TB_Head+@KindName -- Ex: Ori_xls#Consignment_Sale_Order_Car1_3C -- Text 原檔資料
    Set @TB_tmp_Name = @TB_Head+@KindName+'_'+'tmp' -- Ex: Consignment_Sale_Order_Car1_3C_tmp -- 臨時轉入介面
    Set @TB_OR_Name = @TB_Head+@KindName+'_'+@OR_Name -- Ex: Consignment_Sale_Order_Car1_3C_Order   
    Set @Rm = '系統匯入'+@CompanyName+@KindName
    --Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#Consignment_Sale_Order_Car1_3C -- Text 原檔資料
    --Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: Consignment_Sale_Order_Car1_3C_tmp -- 臨時轉入介面
    --Set @TB_OR_Name = @TB_Head+'_'+@OR_Name -- Ex: Consignment_Sale_Order_Car1_Order  
...
print '4'
       ...
       set @Cnt = @Errcode
       set @strSQL = ''
       Set @Msg = '外部 XLS 匯入資料表 ['+@TB_xls_Name+']不存在，終止進行轉檔作業。'
       Exec uSP_Sys_Write_Log @Proc, @Msg, @strSQL, @Cnt

       --close Cur_Mall_Normal_Order_Car1_DataKind
       --deallocate Cur_Mall_Normal_Order_Car1_DataKind
       --Return(@Errcode)
       fetch next from Cur_Mall_Normal_Order_Car1_DataKind into @Kind, @KindName, @OR_Name, @OR_CName, @OR_Class
       Continue
    end
...
print '5'
    set @Cnt = 0
	--Modify by Nan Liao 2015/12/07 配合轉檔時，判斷列印日期欄位
    --set @strSQL = 'Select Distinct F7 as Or_Date1 from [dbo].['+@TB_xls_Name+'] where F2 like ''%列印日期%'' '
    set @strSQL = 'Select Distinct case when F2 like ''%列印日期%'' then F7 else F6 end as Or_Date1 , case when F2 like ''%列印日期%'' then ''1'' else ''2'' end as Or_Datekind from [dbo].['+@TB_xls_Name+'] where F2 like ''%列印日期%'' or F1  like ''%列印日期%'' '
    set @Msg = '判別 Text 製表日期 是否存在'
    Exec uSP_Sys_Write_Log @Proc, @Msg, @strSQL, @Cnt

    delete @RowData
    insert into @RowData exec (@strSQL)
    select @Or_Date1=Rtrim(isnull(aData, '')),@Or_DateKind=Rtrim(isnull(bData, '')) from @RowData
    set @Or_Date1 = Convert(Varchar(10), CONVERT(Date, @Or_Date1) , 111)--由於拉單日期不固定，所以一律改以表單的採購日期為主。
...
print '11'
	   ...
	   begin
		   Set @strSQL = 'select rowid, '+@CR+ -- 列號
						 '       Convert(Varchar(100), '''') as F1, '+@CR+ --原資料
						 '       F3 as F2,  '+@CR+  --店名       f3
						 '       Convert(Varchar(100), '''') as F3,  '+@CR+  --序    
						 '       F8 as F4,  '+@CR+  --商品編號   f8
						 '       F15 as F5,  '+@CR+  --商品名稱   f15
						 '       F27 as F6,  '+@CR+  --訂貨量     f27
						 '       Convert(Varchar(100), '''') as F7,  '+@CR+  --贈品否     
						 '       Convert(Varchar(100), '''') as F8,  '+@CR+  --揀貨量     f29
						 '       F22 as F9,  '+@CR+  --進價       f22
						 '       F29 as F10,  '+@CR+ --小計       f29
						 '       Convert(Varchar(100), '''') as F11,  '+@CR+ --備註       
						 '       '''+@Or_Date1+''' as F12,  '+@CR+ --採購單號   f20 
						 '       F19 as F13,  '+@CR+ --指定到貨日 f19   
						 '       F27 as F14,  '+@CR+ --訂貨量/揀貨量 f27   
						 '       Convert(Varchar(100), '''') as F15,  '+@CR+ --備用計算欄位 f19   
	   ...
	   begin
		   Set @strSQL = 'select rowid, '+@CR+ -- 列號
						 ...
						 '       Convert(Varchar(100), '''') as F3,  '+@CR+  --序    
						 '       F7 as F4,  '+@CR+  --商品編號   f8
						 '       F14 as F5,  '+@CR+  --商品名稱   f15
						 '       F26 as F6,  '+@CR+  --訂貨量     f27
						 '       Convert(Varchar(100), '''') as F7,  '+@CR+  --贈品否     
						 '       Convert(Varchar(100), '''') as F8,  '+@CR+  --揀貨量     f29
						 '       F21 as F9,  '+@CR+  --進價       f22
						 '       F28 as F10,  '+@CR+ --小計       f29
						 '       Convert(Varchar(100), '''') as F11,  '+@CR+ --備註       
						 '       '''+@Or_Date1+''' as F12,  '+@CR+ --採購單號   f20 
						 '       F18 as F13,  '+@CR+ --指定到貨日 f19   
						 '       F26 as F14,  '+@CR+ --訂貨量/揀貨量 f27   
						 '       Convert(Varchar(100), '''') as F15,  '+@CR+ --備用計算欄位 f19   
						 '       print_date='''+@Or_Date1+''', '+@CR+
                         '       xlsFileName, '+@CR+ -- 原始 XLS 檔案名稱
                         '       import_date, '+@CR+ -- 執行 SP_Imp_xls_to_db 匯入日期
       ...
       --*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*
print '12'
       Set @Msg = '抽取店別資料 ['+@TB_xls_Name+']。'
       Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
                     '   set F2=Replace(Replace(REPLACE(F2, '' '', ''''), ''店'', ''''), ''('', ''''), '+@CR+
                     '       F1=''*'''+@CR+
                     ' where F2 not like ''%分店%'''+@CR+
					 '   and F2 > '''''
       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
	   
       Set @Msg = '清除不需要的資料 ['+@TB_tmp_Name+']。'
       Set @strSQL = 'delete from ['+@TB_tmp_Name+']'+@CR+
                     ' where F1 <> ''*'''
       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL

--       Set @Msg = '抽取商品編號資料 ['+@TB_xls_Name+']。'
--       Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
--                     '   set F4=Replace(Replace(REPLACE(F6, '' '', ''''), ''店'', ''''), ''('', ''''), '+@CR+
--                     '       F6='''','+@CR+
--                     '       F1=''*'''+@CR+
--                     ' where F6 not like ''%金額小計%'''+@CR+
--					 '   and F2 = '''''
--       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
       
       Set @Msg = '抽取訂貨量/揀貨量資料 ['+@TB_tmp_Name+']。'
       Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
                     '   set F6=LEFT(rtrim(F14),charindex(''/'',rtrim(F14)) - 1), '+@CR+
                     '       F8=substring(rtrim(F14),charindex(''/'',rtrim(F14)) + 1,len(rtrim(F14)) - charindex(''/'',rtrim(F14))),'+@CR+
                     '       F14='''','+@CR+
                     '       F1=''*'''+@CR+
                     ' where F14 like ''%/%'''
       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
...
--                     '  select rowid, f1 ,f2 ,f3 ,f4 ,f5 ,f6 ,f7 ,f8 ,f9 ,f10,f11,f12,f13,f14,f15 '+@CR+
--                     '    from ['+@TB_tmp_Name+'] '+@CR+
--                     '   order by rowid '+@CR+
--                     ''+@CR+
--                     'declare @rowid int, @cnt int '+@CR+
--                     'declare @idx int '+@CR+
--					 'declare @f1  varchar(100),@vf1  varchar(100)'+@CR+  --原資料        
--					 'declare @f2  varchar(100),@vf2  varchar(100)'+@CR+  --店名       f6 
--					 'declare @f3  varchar(100),@vf3  varchar(100)'+@CR+  --序            
--					 'declare @f4  varchar(100),@vf4  varchar(100)'+@CR+  --商品編號   f6 
--					 'declare @f5  varchar(100),@vf5  varchar(100)'+@CR+  --商品名稱   f13
--					 'declare @f6  varchar(100),@vf6  varchar(100)'+@CR+  --訂貨量     f29
--					 'declare @f7  varchar(100),@vf7  varchar(100)'+@CR+  --贈品否        
--					 'declare @f8  varchar(100),@vf8  varchar(100)'+@CR+  --揀貨量     f29
--					 'declare @f9  varchar(100),@vf9  varchar(100)'+@CR+  --進價       f24
--					 'declare @f10 varchar(100),@vf10 varchar(100)'+@CR+  --小計       f9 
--					 'declare @f11 varchar(100),@vf11 varchar(100)'+@CR+  --備註          
--					 'declare @f12 varchar(100),@vf12 varchar(100)'+@CR+  --採購單號   f20
--					 'declare @f13 varchar(100),@vf13 varchar(100)'+@CR+  --指定到貨日 f19
--					 'declare @f14 varchar(100),@vf14 varchar(100)'+@CR+
--					 'declare @f15 varchar(100),@vf15 varchar(100)'+@CR+
--                     'set @cnt=1 '+@CR+
--                     ''+@CR+
--                     'open cur_book '+@CR+
--                     'fetch next from cur_book into @rowid, @f1 ,@f2 ,@f3 ,@f4 ,@f5 ,@f6 ,@f7 ,@f8 ,@f9 ,@f10,@f11,@f12,@f13,@f14,@f15 '+@CR+
--                     ''+@CR+
--                     'set @cnt=1 '+@CR+
...
print '18'
       ...
       set @strSQL = 'select rowid, ct_no, ct_sname, ct_ssname, '+@CR+
                     ...
                     -- 原商品數量
                     '       Convert(Numeric(20, 6), replace(F6,'','','''')) as Ori_sale_qty, '+@CR+ -- 2015/08/19 Rickliu 保留原本欄位，避免後面修改
                     '       Convert(Numeric(20, 6), replace(F6,'','','''')) as sale_qty, '+@CR+
                     -- 單價
                     '       (Convert(Numeric(20, 6), replace(F10,'','',''''))) / (Convert(Numeric(20, 6), replace(F6,'','',''''))) as Ori_sale_amt, '+@CR+ -- 2015/08/19 Rickliu 保留原本欄位，避免後面修改
                     '       (Convert(Numeric(20, 6), replace(F10,'','',''''))) / (Convert(Numeric(20, 6), replace(F6,'','',''''))) as sale_amt, '+@CR+
                     -- 小計
                     '       (Convert(Numeric(20, 6), replace(F10,'','',''''))) as Ori_sale_tot, '+@CR+ -- 2015/08/19 Rickliu 保留原本欄位，避免後面修改
                     '       (Convert(Numeric(20, 6), replace(F10,'','',''''))) as sale_tot, '+@CR+ 
                     -- 備註
                     '       F11,  '+@CR+ 
                     -- 採購單號
                     '       F12,  '+@CR+ 
...
print '26'
          ...
          set @strSQL ='insert into TYEIPDBS2.lytdbta13.dbo.sorder '+@CR+
                       ...
                       ' , [OR_CTNAME], [OR_SALES], [OR_DPNO],[OR_MSTNO], [OR_MAKER], [OR_TOT] '+@CR+
                       ...
                       '       d2.e_mstno as or_mstno, '+@CR+ -- 業務組別
                       ...
                       '        and ct_class =''1'' '+@CR+					   
                       '       left join SYNC_TA13.dbo.pemploy d2 '+@CR+
                       '         on d1.ct_sales = d2.e_no '+@CR+
                       '       left join  '+@CR+
                       '        (select ct_no,  '+@CR+
...
print '28'
          ...
          --             '        and ct_class =''1'' '+@CR+
          --             '       left join TYEIPDBS2.lytdbta13.dbo.pemploy d2 '+@CR+
          --             '         on d1.ct_sales = d2.e_no '+@CR+
