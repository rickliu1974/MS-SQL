USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_ForMall]    Script Date: 07/24/2017 14:43:59 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_ForMall]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_ForMall]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.ForMall_Order 為 瘋Mall EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_ForMall'
  Declare @Cnt Int =0
  ...
  Set @CompanyName = '瘋Mall' --> 請勿亂變動
    ...
    Set @TB_head = 'EC_Order_ForMall'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_ForMall -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_ForMall_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_ForMall  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_ForMall_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F12:成本      '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F9,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F8 as F5, '+@CR+  --商品名稱
