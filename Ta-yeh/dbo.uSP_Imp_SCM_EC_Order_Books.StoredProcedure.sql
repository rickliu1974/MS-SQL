USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_Books]    Script Date: 08/18/2017 17:43:40 ******/
...
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_Books]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.Books_Order 為 博客來EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_Books'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_Books'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_Books -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_Books_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_Books  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_Books_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F07:訂購量'
PRINT 'F08:收件人'
PRINT 'F09:配送日期(yyyy/mm/dd)'
PRINT 'F10:貨運廠商(請填代碼)'
PRINT 'F11:貨運名稱(當前一欄位選擇其他時，此欄請輸入貨運名稱)'
PRINT 'F12:貨運單號(輸入半形英文或數字)'
PRINT 'F13:聯絡電話'
PRINT 'F14:配送地址'
PRINT 'F15:店內碼'
PRINT 'F16:進貨價'
...
PRINT 'F21:上市日期'
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F6,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F5 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''',F7,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F7 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''',F7,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''',F18,1,1),'' '','''') as F9, '+@CR+  -- 改抓取應付金額
