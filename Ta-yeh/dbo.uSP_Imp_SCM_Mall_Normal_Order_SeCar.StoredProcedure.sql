USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_Mall_Normal_Order_SeCar]    Script Date: 08/18/2017 17:18:56 ******/
...
print '3'
    ...
    Set @TB_xls_Name = @xls_head+@TB_Head+@KindName -- Ex: Ori_xls#Mall_Normal_Order_SeCar_3C -- Text 原檔資料
    Set @TB_tmp_Name = @TB_Head+@KindName+'_'+'tmp' -- Ex: Mall_Normal_Order_SeCar_3C_tmp -- 臨時轉入介面
...
print '5'
    --*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*
    -- 判別 Excel 列印日期/店名 格式是否存在
    -- 由於拉單日期不固定，所以一律改以表單的製表日期為主。
    -- 目前僅支援同一天的製表日期只能轉一次，避免因人員改單造成再次轉檔覆蓋
    set @strSQL = 'Select Distinct SUBSTRING(Convert(Text, F1), 24, 10) as Or_Date1 from [dbo].['+@TB_xls_Name+'] where F1 like ''%日期%'' '
    set @Cnt = 0
    set @Msg = ''
    ...
    set @Msg = '判別 Excel 製表日期...['+@Msg+'存在]'
    Exec uSP_Sys_Write_Log @Proc, @Msg, @strSQL, @Cnt
    

    -- 判別 Excel 店名格式是否存在
    -- 由於旭益拉單時，有時客服會去更改店名，所以有時會將 '-' 的店名漏填，因此在此設個關卡，若只要其中一張單子內的公司名稱有問題，就取消轉檔服務
print '6'
    set @strSQL = 'select all_count - name_count as cnt '+@CR+
                  ...
                  '         where F1 like ''%產品採購單%'' '+@CR+
                  '       ) a, '+@CR+
                  '       (select isnull(Count(1), 0) as name_count '+@CR+
                  '          from '+@xls_head+@TB_head+'Retail '+@CR+
                  '         where F1 like ''%產品採購單%-%店%'' '+@CR+
                  '       ) b'
    set @Cnt = 0
    set @Msg = ''
...
print '7'
       set @Cnt = @Errcode
       set @strSQL = ''
       Set @Msg = '找不到 Excel 資料內的製表日期/店名格式，終止進行轉檔作業。'
       Exec uSP_Sys_Write_Log @Proc, @Msg, @strSQL, @Cnt
...
print '12'
 ...
 where F1 like '%產品採購單%'
   and F2 like '%-%'
       ...
       Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
                     '   set F2=Replace(REPLACE((SELECT items FROM DBO.uFn_Split_StrByDelimiter(F1, ''-'', 1) WHERE items LIKE ''%店%'' ), '' '', ''''), ''店'', '''') '+@CR+
                     ' where F1 like ''%產品採購單%'''
       --Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
       --              '   set F2=Replace(REPLACE(substring(Convert(Text, F1),3,8), '' '', ''''), ''店'', '''') '+@CR+
       --              ' where F1 like ''%統編%'''
       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
...
print '14'
       ...
       Set @strSQL = 'update ['+@TB_tmp_Name+']'+@CR+
                     ...
                     '        F4=Replace(LTrim(RTrim(Substring(Convert(Text, F1), 1, 14))), ''*'', ''''), '+@CR+ --商品編號
                     '        F5=Replace(LTrim(RTrim(Substring(Convert(Text, F21), 1, 100))), '','', ''''), '+@CR+ --商品名稱
                     '        F6=Replace(LTrim(RTrim(Substring(Convert(Varchar(30), F31), 1, 100))), '','', ''''), '+@CR+ --訂貨量
                     '        F7=case when LTrim(RTrim(Substring(Convert(Text, F41), 1, 100))) = ''0'' then ''0'' else ''1'' end, '+@CR+ --贈品否  
                     '        F8=Replace(LTrim(RTrim(Substring(Convert(Text, F31), 1, 100))), '','', ''''), '+@CR+ --揀貨量
                     '        F9=F41, '+@CR+ --進價
                     '        F10=Convert(Numeric(20, 6), PATINDEX(''%[0-9]%'',Convert(Varchar(30), F41))) * Convert(Numeric(20, 6), PATINDEX(''%[0-9]%'',Convert(Varchar(30), F31))), '+@CR+ --小計
                     '        F11=LTrim(RTrim(Substring(Convert(Text, F61), 1, 100)))  '  --備註
       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
...
print '26'
          ...
          set @strSQL ='insert into TYEIPDBS2.lytdbta13.dbo.sorder '+@CR+
                       ...
                       '       1 as or_rate, '+@CR+ -- 匯率
                       '       Substring(F12+'':''+Convert(Varchar(10), rowid), 1, 11) as or_wkno, '+@CR+ -- 客戶訂單單號
                       ...
                       '       Substring(F12+'':''+Convert(Varchar(10), rowid), 1, 11), '+@CR+ -- 客戶訂單單號
                       '       d4.cnt, '+@CR+
                       '       d1.ct_payfg '
          Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
