USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_MyFone]    Script Date: 07/24/2017 14:43:59 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_MyFone]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_MyFone]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.myfone_Order 為 台灣大哥大EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_MyFone'
  Declare @Cnt Int =0
  ...
  Set @CompanyName = '台灣大哥大' --> 請勿亂變動
    ...
    Set @TB_head = 'EC_Order_MyFone'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_MyFone -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_MyFone_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_MyFone_Order  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_MyFone_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F20:出貨單備註'
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F7,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F9 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F12,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F11 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F12,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F13,1,1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F3,1,1),'' '','''') as source_order, '+@CR+ --指定到貨日    
                  '       F15 as buyer, '+@CR+
                  '       F15 as cust, '+@CR+
                  '       F16 as cust_phone, '+@CR+ 
                  '       F17 as cust_mobile, '+@CR+ 
                  '       F18 as cust_addr1, '+@CR+ 
                  '       F18 as cust_addr2, '+@CR+ 
                  '       F20 as MEMO, '+@CR+ 
