USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_GoHappy]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_GoHappy]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_GoHappy]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.GoHappy_Order 為 快樂購EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_GoHappy'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_GoHappy'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_GoHappy -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_GoHappy_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_GoHappy  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_GoHappy_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F24:商品序號        '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F21,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F6 as F5, '+@CR+  --商品名稱
                  '       replace(replace(substring(F23,CHARINDEX(''('',F23)+1,len(f23)),'')'',''''),'''''''','''') as F6, '+@CR+  --訂貨量  
                  '       replace(substring(F23,1,CHARINDEX(''('',F23)-1),'''''''','''') as F7,  '+@CR+  --贈品否  
                  '       replace(replace(substring(F23,CHARINDEX(''('',F23)+1,len(f23)),'')'',''''),'''''''','''') as F8, '+@CR+  --揀貨量  
                  '       replace(substring(F23,1,CHARINDEX(''('',F23)-1),'''''''','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F4,1,1),'' '','''') as source_order, '+@CR+  
                  '       F10 as buyer, '+@CR+
                  '       F10 as cust, '+@CR+
                  '       F12 as cust_phone, '+@CR+ 
                  '       F13 as cust_mobile, '+@CR+ 
                  '       F11 as cust_addr1, '+@CR+ 
                  '       F11 as cust_addr2, '+@CR+ 
                  '       F14 as MEMO, '+@CR+ 
                  '       print_date='''+@Or_Date1+''', '+@CR+
                  '       import_date=convert(datetime, getdate()) '+@CR+
                  '       into ['+@TB_tmp_Name+']'+@CR+
                  '  from ['+@TB_xls_Name+']'+@CR+
                  ' where len(ltrim(rtrim(F23))) > 0'
    Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
