USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_RELO]    Script Date: 08/18/2017 17:18:56 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_RELO]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_RELO]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.RELO_Order 為 台灣利樂EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_RELO_Order'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_RELO'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_RELO -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_RELO_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_RELO  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_Order_RELO_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F16:收件人住址2  '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F6 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F7,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F14 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F7,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F9,1,1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F1,1,1),'' '','''') as source_order, '+@CR+ --原訂單編號    
                  '       F3 as buyer, '+@CR+
                  '       F3 as cust, '+@CR+
                  '       F4 as cust_phone, '+@CR+ 
                  '       F4 as cust_mobile, '+@CR+ 
                  '       F10 as cust_addr1, '+@CR+ 
                  '       F16 as cust_addr2, '+@CR+ 
                  '       F11 as MEMO, '+@CR+ 
