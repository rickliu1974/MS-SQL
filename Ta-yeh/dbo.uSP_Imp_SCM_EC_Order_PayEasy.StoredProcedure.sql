USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_PayEasy]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_PayEasy]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_PayEasy]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.PayEasy_Order 為 PayEasy EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_PayEasy'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_PayEasy'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_PayEasy -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_PayEasy_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_PayEasy  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_PayEasy_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F22:備註          '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F5 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F11,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F9 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F12,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F12 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F12,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F7,1,1),'' '','''') as source_order, '+@CR+ --指定到貨日    
                  '       F15 as buyer, '+@CR+
                  '       F15 as cust, '+@CR+
                  '       F19 as cust_phone, '+@CR+ 
                  '       F21 as cust_mobile, '+@CR+ 
                  '       case when F18 like F17+''%'' then F16 + F18 else F16+F17+F18 end as cust_addr1, '+@CR+
                  '       case when F18 like F17+''%'' then F16 + F18 else F16+F17+F18 end as cust_addr2, '+@CR+
                  '       F22 as MEMO, '+@CR+ 
