USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_MOMO]    Script Date: 07/24/2017 14:43:59 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_MOMO]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_MOMO]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.MOMO_Order 為 富邦MOMO EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_MOMO'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_MOMO'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_MOMO -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_MOMO_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_MOMO -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_MOMO_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
print ' F01:項次+燈號             '
print ' F02:訂單編號              '
print ' F03:配送狀態(說明參考Desc)'
print ' F04:配送訊息              '
print ' F05:約定配送日            '
print ' F06:物流公司(說明參考Desc)'
print ' F07:配送單號              '
print ' F08:訂單類別              '
print ' F09:客戶配送需求          '
print ' F10:轉單日                '
...
print ' F28:群組變價商品          '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F1 as F3, '+@CR+  --序      
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F16,1,1),'' '','''') as F4, '+@CR+  --商品編號
                  '       F18 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F21,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F22,1,1),'' '','''') as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F21,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F22,1,1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       F2 as source_order, '+@CR+ --訂單編號  
                  '       F24 as buyer, '+@CR+
                  '       F12 as cust, '+@CR+
                  '       F13 as cust_phone, '+@CR+ 
                  '       F14 as cust_mobile, '+@CR+ 
                  '       F15 as cust_addr1, '+@CR+ 
                  '       F15 as cust_addr2, '+@CR+ 
                  '       F28 as MEMO, '+@CR+ 
