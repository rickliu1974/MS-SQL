USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_Stock_taking_GZ]    Script Date: 07/24/2017 14:44:00 ******/
DROP PROCEDURE [dbo].[uSP_Imp_Stock_taking_GZ]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_Stock_taking_GZ]
as
begin
  /***********************************************************************************************************
     2013/12/06 -- Rickliu
  ...
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_Stock_taking_GZ'
  Declare @Cnt Int =0
...
print '1'
  ...
  Set @TB_head = 'Stock_taking_GZ'
  Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#Stock_taking_GZ -- Excel 原檔資料
  Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: Stock_taking_GZ_tmp -- 臨時轉入介面
  Set @TB_SP_Name = @TB_Head -- Ex: Stock_taking_GZ
  set @SP_Class = '7' -- 盤點類
...
print '7'
  select @SP_cnt = sum(cnt)
    from (select count(1) as cnt
           from TYEIPDBS2.lytdbGZ.dbo.sslip m
         ...
         select count(1) as cnt
           from TYEIPDBS2.lytdbGZ.dbo.ssliptmp m
          where SP_class = @SP_class
            and sp_slip_fg = @sp_slip_fg
            and SP_date = @SP_date
...
print '25'
        ...
        set @strSQL ='delete TYEIPDBS2.lytdbGZ.dbo.sslpdttmp '+@CR+
...
print '26'
        ...
        set @strSQL ='delete TYEIPDBS2.lytdbGZ.dbo.ssliptmp '+@CR+
...
print '27'
        ...
        set @strSQL ='insert into TYEIPDBS2.lytdbGZ.dbo.sslpdttmp '+@CR+
                     ...
                     '       inner join TYEIPDBS2.lytdbGZ.dbo.sstock d '+@CR+
                     ...
                     '            from TYEIPDBS2.lytdbGZ.dbo.swaredt m'+@CR+
                     '                 inner join '+@CR+
                     '                   (select wd_no, wd_skno, max(wd_yr) as wd_yr '+@CR+
                     '                      from TYEIPDBS2.lytdbGZ.dbo.swaredt '+@CR+
...
print '29'
        ...
        set @strSQL ='insert into TYEIPDBS2.lytdbGZ.dbo.ssliptmp '+@CR+
                     ...
                     '  from TYEIPDBS2.lytdbGZ.dbo.sslpdttmp m '+@CR+
                     '       left join TYEIPDBS2.lytdbGZ.dbo.sstock d '+@CR+
