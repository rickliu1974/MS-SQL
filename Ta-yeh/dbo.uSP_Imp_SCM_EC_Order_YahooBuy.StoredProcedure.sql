USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Order_YahooBuy]    Script Date: 08/18/2017 17:43:40 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Order_YahooBuy]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Order_YahooBuy]
as
begin
  /***********************************************************************************************************
     2017/04/17 Rickliu
     1.YahooBuy_Order 為 Yahoo購物EC平台轉檔 平台使用。
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Order_YahooBuy'
  Declare @Cnt Int =0
  ...
  Begin Try
    ...
    Set @TB_head = 'EC_Order_YahooBuy'
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_xls#EC_Order_YahooBuy -- 外部原檔匯入資料
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Order_YahooBuy_tmp -- 臨時轉入處理介面檔
    Set @TB_OR_Name = @TB_Head -- Ex: EC_Order_YahooBuy  -- 待轉入凌越資料介面檔
    Set @TB_Chk_Order = '##Chk_'+@TB_Head+'_tmp' -- Ex: ##Chk_EC_Order_YahooBuy_tmp --< 此資料存放於 tempdb，程序離開時會清除。
...
PRINT 'F19:合併序號      '
    ...
    Set @strSQL = 'select rowid, '+@CR+ -- 列號
                  ...
                  '       F13 as F5, '+@CR+  --商品名稱
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F16,1,1),'' '','''') as F6, '+@CR+  --訂貨量  
                  '       F16 as F7, '+@CR+  --贈品否  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F16,1,1),'' '','''') as F8, '+@CR+  --揀貨量  
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z.0-9]'', '''', F17,1,1),'' '','''') as F9, '+@CR+  --進價    
                  ...
                  '       replace(dbo.uFn_RegexReplace(''[^A-Z0-9]'', '''', F1,1,1),'' '','''') as source_order, '+@CR+ --指定到貨日    
                  '       F3 as buyer, '+@CR+
                  '       F3 as cust, '+@CR+
                  '       F4 as cust_phone, '+@CR+ 
                  '       F6 as cust_mobile, '+@CR+ 
                  '       F7+F8 as cust_addr1, '+@CR+ 
                  '       F7+F8 as cust_addr2, '+@CR+ 
                  '       F18 as MEMO, '+@CR+ 
