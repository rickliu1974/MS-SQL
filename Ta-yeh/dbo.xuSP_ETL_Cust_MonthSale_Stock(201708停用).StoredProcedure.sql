USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[xuSP_ETL_Cust_MonthSale_Stock(201708停用)]    Script Date: 08/18/2017 17:43:41 ******/
DROP PROCEDURE [dbo].[xuSP_ETL_Cust_MonthSale_Stock(201708停用)]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[xuSP_ETL_Cust_MonthSale_Stock(201708停用)]
as
begin
  /*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
  ...
  *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/  
  ...
  IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Cust_MonthSale_Stock]') AND type in (N'U'))
  begin
     Set @Msg = '刪除資料表 [Cust_MonthSale_Stock]'
     set @strSQL= 'DROP TABLE [dbo].[Cust_MonthSale_Stock]'

     Exec @Result = uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
     if @Result <>  -1 Set @Result = 0
  end

  set @Msg = 'Create to [Cust_MonthSale_Stock]...'
  begin try
    select *,
			  ...
			  from (select '1' as kind, '已銷' as kind_Name, ct_no8, ct_sname8, chg_ct_fld3, -- 2017/02/08 Rickliu Add 增加八碼客戶編號
               ...
               from (select *
                    ...
                    --  and t.sd_date BETWEEN d.cal_week_begin AND cal_week_end 
                     left join Fact_sstock as s
                       on t.sd_skno=s.sk_no
              where 1=1
     ...
     update Cust_MonthSale_Stock
        set all_count_ctno = isnull(d.all_count_ctno,0),
            all_count_skno = isnull(d.all_count_skno,0)
       from Cust_MonthSale_Stock m
            left join 
            (select Chg_sp_sales, Chg_sp_date_YM,
                    Count(Distinct ct_no8) as all_count_ctno,
                    Count(Distinct sd_skno) as all_count_skno
               from Cust_MonthSale_Stock
              group by Chg_sp_sales,Chg_sp_date_YM
            ) d on m.Chg_sp_sales = d.Chg_sp_sales and m.Chg_sp_date_YM=d.Chg_sp_date_YM
     ...
     select @Cnt = count(1) from Cust_MonthSale_Stock
