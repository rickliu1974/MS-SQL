USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_Comp_EC_Consign_Stock_PChome]    Script Date: 08/18/2017 17:18:56 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_Comp_EC_Consign_Stock_PChome]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_Comp_EC_Consign_Stock_PChome]
as
begin
  /***********************************************************************************************************
     ...
     2013/11/28 增加失敗回傳值
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_Comp_EC_Consign_Stock_PChome'
  Declare @Cnt Int =0
  ...
  Set @CompanyLikeCode = 'I90010013'
  Set @CompanyName = 'PCHOME'
...
print '4'
  ...
  Set @TB_xls_Name = Isnull(@xls_head+@TB_Head, '') -- Ex: Ori_Xls#Comp_EC_Consign_Stock_Pchome -- Excel 原檔資料
  Set @TB_tmp_Name = Isnull(@TB_Head, '')+'_tmp' -- Ex: Comp_EC_Consign_Stock_tmp -- 臨時轉入介面
...
print '6'
  ...
  set @strSQL = 'Select Top 1 substring(SplitFileName, 1, 8) as Print_Date from [dbo].['+@TB_xls_Name+']  '
...
print '19'
    ...
    20151025_Comp_EC_Consign_Stock_PChome 格式:
    F01:供應商料號(V) F02:商品ID(V)       F03:品名(V)      F04:期初庫存      F05:進貨數(V)
    F06:還貨數        F07:銷貨數          F08:客退數       F09:庫存
    
    20170621_Comp_EC_Consign_Stock_PChome 格式:
    F01:供應商料號(V) F02:PCHOME商品ID(V) F03:品名(V)      F04:規格          F05:總在庫數
    F06:良品          F07:壞品            F08:尚未判定     F09:平均庫齡      F10:在庫金額
    F11:未到驗借貨單
    *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
    Set @Msg = '進行比對商品編號並寫入 ['+@TB_OD_Name+']資料表。' 
    set @strSQL = 
              'With CTE_Q1 as ( '+@CR+
              '  Select F1 as xls_skno, '+@CR+ -- F01:供應商料號(Excel)
              '         F3 as xls_skname, '+@CR+ -- F03: 品名(Excel)
              '         F2 as xls_cono, '+@CR+ -- F02: 商品ID(Excel)
              '         Sum(isnull(Convert(Numeric(20, 6), Replace(F5, '','', '''')), 0)) as xls_qty '+@CR+ -- F09:寄賣庫存 (Excel) 改 V20170621.F05 總在庫數
              '    from '+@TB_tmp_Name+@CR+
              '   Group by F1, F3, F2 '+@CR+
              '), CTE_Q2 as ( '+@CR+
              '  Select distinct '+@CR+ 
