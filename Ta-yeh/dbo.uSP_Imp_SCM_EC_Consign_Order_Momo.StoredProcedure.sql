USE [DW]
GO
/****** Object:  StoredProcedure [dbo].[uSP_Imp_SCM_EC_Consign_Order_Momo]    Script Date: 07/24/2017 14:43:59 ******/
DROP PROCEDURE [dbo].[uSP_Imp_SCM_EC_Consign_Order_Momo]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Proc [dbo].[uSP_Imp_SCM_EC_Consign_Order_Momo]
as
begin
  /***********************************************************************************************************
     ...
     -- 2013/11/28 增加失敗回傳值
  ***********************************************************************************************************/
  Declare @Proc Varchar(50) = 'uSP_Imp_SCM_EC_Consign_Order_Momo'
  Declare @Cnt Int =0
  ...
  Set @CompanyName = '富邦MOMO' --> 請勿亂變動
  Set @CompanyLikeName = '%'+@CompanyName+'%'
  Set @Rm = '系統匯入'+@CompanyName
  Set @sp_maker = 'Admin'

print '1'
  Declare Cur_Mall_Consign_Order_Momo_DataKind cursor for
    select *
  ...
  open Cur_Mall_Consign_Order_Momo_DataKind
  fetch next from Cur_Mall_Consign_Order_Momo_DataKind into @OD_Name, @OD_CName, @sd_class, @sd_slip_fg, @oper
...
print '3'
    ...
    Set @TB_xls_Name = @xls_head+@TB_Head -- Ex: Ori_Xls#EC_Consign_Order_Momo_Momo -- Excel 原檔資料 PS: 這個檔案是與 SP_Comp_Sale_Order_Momo 共用檔案
    Set @TB_tmp_Name = @TB_Head+'_'+'tmp' -- Ex: EC_Consign_Order_Momo_Momo_tmp -- 臨時轉入介面
    Set @TB_OD_Name = @TB_Head+'_'+@OD_Name -- Ex: EC_Consign_Order_Momo_Reject  
...
print '5'
    ...
    set @strSQL = 'Declare @YM Varchar(7) '+@CR+
                  ...
                  */
                  ...
                  -- 2015/04/07 由於之前使用系統月導致無法控制已經匯過的資料會重複匯入，所以改為 imp_date
                  'select top 1 @YM = Convert(Varchar(7), DateAdd(mm, -1, imp_date), 111) '+@CR+
                  '  from '+@TB_xls_name+@CR+
                  ' '+@CR+
...
print '6'
       ...
       Exec uSP_Sys_Write_Log @Proc, @Msg, @strSQL, @Cnt
       -- 2013/11/28 增加失敗回傳值
       close Cur_Mall_Consign_Order_Momo_DataKind
       deallocate Cur_Mall_Consign_Order_Momo_DataKind
       Return(@Errcode)
...
print '10'
       ...
       --將資料進行分類。
print '11'
       Set @Msg = '將資料進行分類 ['+@TB_tmp_Name+']。'
       Set @strSQL = 'select *, '+@CR+
                     '       case '+@CR+
                     --'         when F5 like ''寄倉%'' and (F14 like ''%快速到貨%'' or F14 like ''%快-加價購%'') then ''7'' '+@CR+ -- 轉託售單
                     '         when F5 like ''寄倉%'' then ''7'' '+@CR+ -- 轉託售單
                     --'         when F6 like ''退貨%'' and (F14 like ''%快速到貨%'' or F14 like ''%快-加價購%'') then ''3'' '+@CR+-- 轉退貨單。
                     '         when F6 like ''退貨%'' then ''3'' '+@CR+-- 轉退貨單。
                     '         else '''' '+@CR+
                     '       end slip_fg, '+@CR+
                     '       Substring(F2, 1, 15) as Order_no, '+@CR+
                     '       Convert(Varchar(7), F8) as sp_pdate, '+@CR+
                     '       Rtrim(Isnull(F11, '''')) as XLS_CT_Name, '+@CR+
                     '       Rtrim(Isnull(F13, ''''))+Rtrim(Isnull(F15, '''')) as XLS_SKNO, '+@CR+
                     '       Convert(Int, Isnull(F17, 0)) as XLS_Qty, '+@CR+ -- 數量
                     '       Convert(Int, Isnull(F18, 0)) as XLS_Amt, '+@CR+ -- 進價(未稅) 
                     '       Convert(Int, Isnull(F17, 0)) * Convert(Int, Isnull(F18, 0)) as XLS_Tot, '+@CR+ -- 小計(未稅) 
                     '       YM='''+@YM+''', '+@CR+
                     '       B_Pdate='''+@B_PDate+''', '+@CR+
                     '       E_Pdate='''+@E_PDate+''', '+@CR+
                     ...
                     --'   and (F5 like ''寄倉%'' and (F14 like ''%快速到貨%'' or F14 like ''%快-加價購%'')) '+@CR+
                     '   and (F5 like ''寄倉%'' ) '+@CR+
                     --'    or (F6 like ''退貨%'' and (F14 like ''%快速到貨%'' or F14 like ''%快-加價購%'')) '
                     '    or (F6 like ''退貨%'' ) '
       print '@B_PDate='+@B_PDate
       print '@E_Pdate='+@E_PDate
       print '@TB_tmp_Name='+@TB_tmp_Name
       print '@TB_xls_Name='+@TB_xls_Name
       print @strSQL          
       Exec uSP_Sys_Exec_SQL @Proc, @Msg, @strSQL
...
print '21'
       ...
       set @strSQL = 'select rowid, ct_no, ct_sname, ct_ssname, '+@CR+
                     ...
                     '       IsNull(XLS_AMT, 0) as Sale_amt, '+@CR+ -- 單價
